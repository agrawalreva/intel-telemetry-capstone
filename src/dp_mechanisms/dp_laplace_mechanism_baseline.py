...
def run_laplace_mechanism(database: str = "mini",
    ...
    for query_num, meta in QUERY_META.items():
        ...
        for eps_idx, epsilon in epsilons_to_run:
            ...
            # Base folder: data/dp_laplace_mini or data/dp_laplace_full
            base_out = (
    ...
    summary_dir = (
        os.path.join(os.path.dirname(BASELINE_MINI), "dp_laplace_mini", "baseline")
        if database == "mini"
        else os.path.join(os.path.dirname(BASELINE_FULL), "dp_laplace_full", "baseline")
    )
