"""
dp_laplace_mechanism_baseline.py

Baseline Laplace DP mechanism — naive (per-query, per-epsilon) approach.

For every query and every epsilon value, the noise scale (b = Δ₁/ε) is
computed independently inside the column loop.  This is the simplest correct
implementation and serves as the reference for comparison with the advance
version.

Key difference from advance version:
  BASELINE  → scale re-computed for every (query, column, epsilon) triple
  ADVANCE   → scale pre-computed once per unique (sensitivity, epsilon) pair
...
2) For each epsilon (seeded rng = RANDOM_SEED + eps_idx):
   a) For each numeric column, compute scale b = Δ₁/ε fresh.
   ...
   e) Save noisy CSV to data/dp_laplace_mini/baseline/eps_<epsilon>/
   f) Append row to running summary DataFrame.
3) Save summary to data/dp_laplace_mini/baseline/laplace_metric_summary.csv

Usage:
    cd scripts/dp_mechanisms
    python dp_laplace_mechanism_baseline.py --database mini
    python dp_laplace_mechanism_baseline.py --database full
"""
...
def compute_metric_RE(true_df: pd.DataFrame,
    ...
    true_v, noisy_v = true_vals.values[:n], noisy_vals.values[:n]
    nonzero = true_v != 0
    if nonzero.sum() == 0:
        median_re = float("nan")
    else:
...
def post_process(noisy_df: pd.DataFrame, meta: dict) -> pd.DataFrame:
    ...
    3. Re-normalise per-row distribution columns (Q7, Q8).
    """
    df = noisy_df.copy()
...
# =============================================================================
#  CORE: apply Laplace DP to one query CSV  (BASELINE — naive per-column loop)
# =============================================================================
...
def apply_laplace_dp(
    ...
    """
    BASELINE version — scale b = Δ₁/ε computed fresh for every column on every call.
    ...
    # ------------------------------------------------------------------
    # BASELINE: scale computed fresh for every column (naive loop).
    # ------------------------------------------------------------------
    for col in numeric_cols:
        ...
        # ← BASELINE: scale re-computed every time (no caching)
        scale = laplace_scale(col_sens, col_epsilon)

        print(f"    [Laplace:{col}] sens={col_sens:.4f}  "
              f"eps_col={col_epsilon:.4f}  scale={scale:.4f}")
...
def run_laplace_baseline(database: str = "mini") -> None:
    """
    Baseline pipeline — naive per-query, per-epsilon noise calculation.
    Outputs go to data/dp_laplace_{mini|full}/baseline/
    """
    ...
    print("\n" + "=" * 70)
    print("LAPLACE BASELINE COMPLETE")
    print("=" * 70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Baseline Laplace DP mechanism — naive per-query loop."
    )
    parser.add_argument(
    ...
    run_laplace_baseline(database=args.database)